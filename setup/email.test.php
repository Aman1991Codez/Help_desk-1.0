...
$oP = new SetupPage('JSOFT_SOLUTION email test utility');
...
function CheckEmailSetting($oP)
{
	...
	{
		case 'PHPMail':
		$oP->info("JSOFT_SOLUTION is configured to use PHP's <a style=\"background:transparent;padding:0;color:#000;text-decoration:underline;\" href=\"http://www.php.net/manual/en/function.mail.php\" target=\"_blank\">mail</a> function to send emails.");
		...
		case 'SMTP':
		$oP->info("JSOFT_SOLUTION is configured to use the <b>$sTransport</b> transport.");
		...
		{
			$oP->warning("The default settings may not be suitable for your environment. You may want to adjust these values by editing JSOFT_SOLUTION configuration file (".utils::GetConfigFilePathRelative().").");
		}
		break;
		...
		case 'SMTP_OAuth':
			...
			try {
				...
				if (($sHost == 'localhost') && ($sPort == '25') && ($sUserName == '') && ($sClientID == '') && ($sProvider == '')) {
					$oP->warning("The default settings may not be suitable for your environment. You may want to adjust these values by editing JSOFT_SOLUTION's configuration file (".utils::GetConfigFilePathRelative().').');
				}
		...
		case 'Null':
		$oP->warning("JSOFT_SOLUTION is configured to use the <b>Null</b> transport: emails sending will have no effect.");
		$bRet = false;
		break;
		
		case 'LogFile':
		$oP->warning("JSOFT_SOLUTION is configured to use the <b>LogFile</b> transport: emails will <em>not</em> be sent but logged to the file: 'log/mail.log'.");
		$bRet = true;
		break;
	...
	if ($bConfigAsync)
	{
		$oP->warning("JSOFT_SOLUTION is configured to send emails <em>asynchronously</em>. Make sure that cron.php is scheduled to run in the background, otherwise regular emails will <em>not</em> be sent. For the purpose of this test, the email will be sent <em>synchronously</em>.");
	}
...
function DisplayStep1(SetupPage $oP)
{
	...
	$oP->add("<h1>JSOFT_SOLUTION email test</h1>\n");
...
function DisplayStep2(SetupPage $oP, $sFrom, $sTo)
{
	...
	$oP->add("<h1>JSOFT_SOLUTION configuration wizard</h1>\n");
	...
	$oEmail->SetSubject(" JSOFT SOLUTION ");
	$oEmail->SetBody("<p>IT HELPDESK,</p><p>WELCOME TO,  JSOFT_SOLUTION Online IT Helpdesk Ticketing System.<br>
	 We are always ready to serve you best,
	 The email function is now working fine.</p><p>Operation Hours:  Mon-Friday(9am - 6pm) </p>
	 <p>url links: help.jsoftsolution.com.my </p><p>JSOFT_SOLUTION</p>");

...
class AjaxController extends Controller
{
	public function OperationCanUpdateCore()
	{
		...
		try
		{
			...
			if ($bCanUpdateCore)
			{
				$aParams['sMessage'] = Dict::S('JSOFT_SOLUTION_Update:UI:CanCoreUpdate:Yes');
			}
			else
			{
				$sLink = utils::GetAbsoluteUrlAppRoot().'setup/';
				$sLinkManualUpdate = 'https://www.JSOFT_SOLUTION.io/wiki/page?id='.utils::GetItopVersionWikiSyntax().'%3Ainstall%3Aupgrading_JSOFT_SOLUTION_#manually';
				$aParams['sMessage']  = Dict::Format('JSOFT_SOLUTION_Update:UI:CannotUpdateUseSetup', $sLink, $sLinkManualUpdate);
				$aParams['sMessageDetails']  = $sMessage;
			}
		} catch (FileNotExistException $e)
		{
			$aParams['bStatus'] = false;
			$aParams['sMessage'] = Dict::Format('JSOFT_SOLUTION_Update:UI:CanCoreUpdate:ErrorFileNotExist', $e->getMessage());
		} catch (Exception $e)
		{
			$aParams['bStatus'] = false;
			$aParams['sMessage'] = Dict::Format('JSOFT_SOLUTION_Update:UI:CanCoreUpdate:Error', $e->getMessage());
		}

		$this->DisplayJSONPage($aParams);
	}

	public function OperationGetJSOFT_SOLUTION_DiskSpace()
	{
		$aParams = array();

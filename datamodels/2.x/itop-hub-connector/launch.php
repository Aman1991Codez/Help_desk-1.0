...
function collect_configuration()
{
	...
	// PHP extensions
	if (!MetaModel::GetConfig()->GetModuleSetting('JSOFT_SOLUTION-hub-connector', 'php_extensions_enable', true)) {
	...
	$aIniGet = MetaModel::GetConfig()->GetModuleSetting('JSOFT_SOLUTION-hub-connector', 'php_settings_array', array()); // by default, on the time of the writting, it values are : array('post_max_size', 'upload_max_filesize', 'apc.enabled', 'timezone', 'memory_limit', 'max_execution_time');
	...
	foreach ($oExtensionMap->GetChoices() as $oExtension) {
		switch ($oExtension->sSource) {
			...
			default:
				$aConfiguration['JSOFT_SOLUTION_installation_options'][$oExtension->sCode] = array(
					'label' => $oExtension->sLabel,
					'value' => true,
				);
		}
...
function MakeDataToPost($sTargetRoute)
{
	if (MetaModel::GetConfig()->Get('demo_mode')) {
		...
		$aDataToPost = array(
			'JSOFT_SOLUTION_hub_target_route' => $sTargetRoute,
			'JSOFT_SOLUTION_stack' => array(
				...
				'application_version_full' => (string)Dict::Format('UI:JSOFT_SOLUTIONVersion:Long', ITOP_APPLICATION, ITOP_VERSION, ITOP_REVISION, ITOP_BUILD_DATE),
				'JSOFT_SOLUTION_user_id' => (string)(UserRights::GetUserId() === null) ? "1" : UserRights::GetUserId(),
				'JSOFT_SOLUTION_user_lang' => (string)UserRights::GetUserLanguage(),
				'JSOFT_SOLUTION_modules' => (object)$aConfiguration['JSOFT_SOLUTION_modules'],
				'JSOFT_SOLUTION_extensions' => (object)$aConfiguration['JSOFT_SOLUTION_extensions'],
				'JSOFT_SOLUTION_installation_options' => (object)$aConfiguration['JSOFT_SOLUTION_installation_options'],
			),
			'server_stack' => array(
...
try {
	...
	$sHubUrlStateless = MetaModel::GetModuleSetting('JSOFT_SOLUTION-hub-connector', 'url').MetaModel::GetModuleSetting('JSOFT_SOLUTION-hub-connector', 'route_landing_stateless');
	$sHubUrl = MetaModel::GetModuleSetting('JSOFT_SOLUTION-hub-connector', 'url').MetaModel::GetModuleSetting('JSOFT_SOLUTION-hub-connector', 'route_landing');
	...
	switch ($sTargetRoute) {
		...
		default:
			...
			if ($sTargetRoute == 'view_dashboard') {
				$sTitle = Dict::S('Menu:JSOFT_SOLUTION_Hub:Register');
				$sLabel = Dict::S('Menu:JSOFT_SOLUTION_Hub:Register+');
				$sText = Dict::S('Menu:JSOFT_SOLUTION_Hub:Register:Description');
			} else {
				$sTitle = Dict::S('Menu:JSOFT_SOLUTION_Hub:BrowseExtensions');
				$sLabel = Dict::S('Menu:JSOFT_SOLUTION_Hub:BrowseExtensions+');
				$sText = Dict::S('Menu:JSOFT_SOLUTION_Hub:BrowseExtensions:Description');
			}
			$sLogoUrl = utils::GetAbsoluteUrlModulesRoot().'/JSOFT_SOLUTION-hub-connector/images/itophub-logo.svg';
			...
			$oPage = new HubConnectorPage(Dict::S('JSOFT_SOLUTION_Hub:Connect'));
			...
			if (MetaModel::GetConfig()->Get('demo_mode')) {
				$oPage->add("<div class=\"header_message message_info\">Sorry, JSOFT_SOLUTION is in <b>demonstration mode</b>: the connection to JSOFT_SOLUTION Hub is disabled.</div>");
			}
			...
			$oPage->add($sText);
			$sButtonLabelClose = Dict::S('JSOFT_SOLUTION_Hub:CloseBtn');
			$sButtonLabelGo = Dict::S('JSOFT_SOLUTION_Hub:GoBtn');
			$sButtonLabelTooltip = Dict::S('JSOFT_SOLUTION_Hub:GoBtn:Tooltip');
			$oPage->add(<<<HTML
<p>
<button type="button" class="ibo-button" id="CancelBtn" title="Go back to JSOFT_SOLUTION"><img src="$sCloseUrl"><span class="ibo-button--label">$sButtonLabelClose</span></button>
...
HTML
			...
			$sFormTarget = appUserPreferences::GetPref('JSOFT_SOLUTION_hub_open_in_new_window', 1) ? 'target="_blank"' : '';
			...
			$sAutoSubmitChecked = appUserPreferences::GetPref('JSOFT_SOLUTION_hub_auto_submit', 0) == 1 ? 'checked' : '';
			$oPage->add('<label><input type="checkbox" class="userpref" id="JSOFT_SOLUTION_hub_auto_submit" '.$sAutoSubmitChecked.'>&nbsp;'.Dict::S('JSOFT_SOLUTIONHub:AutoSubmit').'</label></p>');
			$oPage->add('</form>');

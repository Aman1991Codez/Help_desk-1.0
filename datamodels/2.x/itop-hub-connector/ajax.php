...
function DoBackup($sTargetFile)
{
	...
	$oBackup->SetMySQLBinDir(MetaModel::GetConfig()->GetModuleSetting('JSOFT_SOLUTION-backup', 'mysql_bindir', ''));
	...
	switch ($sOperation)
	{
		case 'check_before_backup':
			...
			if (!$bFailed)
			{
				...
				if ($fFreeSpace!==false)
				{
					$sMessage = Dict::Format('JSOFT_SOLUTIONHub:BackupFreeDiskSpaceIn', SetupUtils::HumanReadableSize($fFreeSpace), dirname($sDBBackupPath));
					ReportSuccess($sMessage);
				}
				else
				{
					ReportError(Dict::S('JSOFT_SOLUTIONHub:FailedToCheckFreeDiskSpace'), -1);
				}
			}
		break;
		...
		case 'do_backup':
			...
			try
			{
				...
				if (count($aErrors)>0)
				{
				    ...
				    ReportError(Dict::S('JSOFT_SOLUTIONHub:BackupFailed'), -1, $aErrors);
				}
				else
				{
					SetupLog::Info('Backup successfully completed.');
					ReportSuccess(Dict::S('JSOFT_SOLUTIONHub:BackupOk'));
				}
			}
			catch (Exception $e)
			{
		...
		case 'compile':
			...
			{
					throw new SecurityException(Dict::S('JSOFT_SOLUTIONHub:FailAuthent'));
			}
		...
		case 'move_to_production':
			...
			try
			{
				...
				$oRuntimeEnv->RecordInstallation($oConfig, $sDataModelVersion, $aSelectedModules, $aSelectedExtensionCodes, 'Done by the JSOFT_SOLUTION Hub Connector');

				// Report the success in a way that will be detected by the ajax caller
				SetupLog::Info('Deployment successfully completed.');
				ReportSuccess(Dict::S('JSOFT_SOLUTIONHub:CompiledOK'));
			}
			catch (Exception $e)
			{
				...
				SetupLog::Error(get_class($e).': '.Dict::S('JSOFT_SOLUTIONHub:ConfigurationSafelyReverted')."\n".$e->getMessage());

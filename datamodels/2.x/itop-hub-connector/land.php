...
function DisplayStatus(WebPage $oPage)
{
	...
	$oPage->add('<td><h2>'.Dict::S('JSOFT_SOLUTION:LandingWelcome').'</h2><td>');
	$oPage->add('</tr></table>');
	...
	foreach ($oExtensionsMap->GetAllExtensions() as $oExtension) {
		if ($oExtension->sSource == iTopExtension::SOURCE_REMOTE) {
			...
			if ($oExtension->sInstalledVersion === '') {
				...
				$sInstallation = Dict::Format('JSOFT_SOLUTION:InstallationStatus:Version_NotInstalled', $oExtension->sVersion);

			} else {
				$aCSSClasses[] = 'landing-no-change';
				$sBadge = '<span style="display:inline-block;font-size:8pt;padding:3px;border-radius:4px;color:#fff;background-color:#1c94c4;margin-left:0.5em;margin-right:0.5em">'.Dict::S('JSOFT_SOLUTION:InstallationStatus:Installed').'</span>';
				$sInstallation = Dict::Format('JSOFT_SOLUTION:InstallationStatus:Installed_Version', $sBadge, $oExtension->sInstalledVersion);
			}
	...
	$oPage->add('</div>');
	$oPage->add('<div style="text-align:center"><button onclick="window.location.href=\'./UI.php\';">'.Dict::S('JSOFT_SOLUTION:GoBackToITopBtn').'</button></div>');
}
...
function DoLanding(WebPage $oPage)
{
	...
	$sBannerUrl = utils::GetAbsoluteUrlModulesRoot().'/itop-hub-connector/images/landing-extension.png';
	$oPage->add('<td><img style="max-height:72px; margin-right: 10px;" src="'.$sBannerUrl.'"/><td>');
	$oPage->add('<td><h2>'.Dict::S('JSOFT_SOLUTION:InstallationWelcome').'</h2><td>');
	$oPage->add('</tr></table>');

	$oPage->set_title(Dict::S('JSOFT_SOLUTION:Landing:Status'));

	$oPage->add('<div class="module-selection-body" style="text-align: center; line-height: 14em;"><h2>'.Dict::S('JSOFT_SOLUTION:Uncompressing').'</h2></div>');
...
function DoInstall(WebPage $oPage)
{
	...
	$oPage->set_title(Dict::S('JSOFT_SOLUTION:Landing:Install'));
	...
	foreach ($oExtensionsMap->GetAllExtensions() as $oExtension) {
		if ($oExtension->sSource == iTopExtension::SOURCE_REMOTE) {
			if (count($oExtension->aMissingDependencies) > 0) {
				...
				$sTitle = Dict::Format('JSOFT_SOLUTION:InstallationEffect:MissingDependencies_Details', implode(', ', $oExtension->aMissingDependencies));
				$oPage->add('<label><b>'.utils::EscapeHtml($oExtension->sLabel).'</b> <span style="color:red" title="'.$sTitle.'">'.Dict::S('JSOFT_SOLUTION:InstallationEffect:MissingDependencies').'<span></label>');
				$oPage->add('<div class="description">');
				$oPage->add('<p>');
				if ($oExtension->sDescription != '') {
					$oPage->add(utils::EscapeHtml($oExtension->sDescription).'</br>');
				...
				if ($oExtension->sInstalledVersion === '') {
					$aCSSClasses[] = 'landing-installation';
					$sInstallation = Dict::Format('JSOFT_SOLUTION:InstallationEffect:Install', $oExtension->sVersion);
				} else if ($oExtension->sInstalledVersion == $oExtension->sVersion) {
					$aCSSClasses[] = 'landing-no-change';
					$sInstallation = Dict::Format('JSOFT_SOLUTION:InstallationEffect:NoChange', $oExtension->sVersion);
				} else if (version_compare($oExtension->sInstalledVersion, $oExtension->sVersion, '<')) {
					$aCSSClasses[] = 'landing-upgrade';
					$sInstallation = Dict::Format('JSOFT_SOLUTION:InstallationEffect:Upgrade', $oExtension->sInstalledVersion, $oExtension->sVersion);
				} else {
					$aCSSClasses[] = 'landing-downgrade';
					$sInstallation = Dict::Format('JSOFT_SOLUTION:InstallationEffect:Downgrade', $oExtension->sInstalledVersion, $oExtension->sVersion);
				}
				$oPage->add('<div class="choice">');
				$sCode = $oExtension->sCode;
				$sDir = basename($oExtension->sSourceDir);
	...
	$oPage->add('<div id="hub-installation-progress-text">'.Dict::S('JSOFT_SOLUTION:DatabaseBackupProgress').'</div>');
	...
	$oPage->add('<fieldset id="database-backup-fieldset"><legend>'.Dict::S('JSOFT_SOLUTION:DBBackupLabel').'</legend>');
	$oPage->add('<div id="backup_form"><input id="backup_checkbox" type="checkbox" checked><label for="backup_checkbox"> '.Dict::S('JSOFT_SOLUTION:DBBackupSentence').'</label></div>');
	$oPage->add('<div id="backup_status"></div>');
	$oPage->add('</fieldset>');
	$oPage->add('<p style="text-align: center"><input type="button" id="hub_start_installation" type="button" disabled value="'.Dict::S('JSOFT_SOLUTION:DeployBtn').'"/></p>');
	...
	$aWidgetParams = array(
		...
		'labels' => array(
			'database_backup' => Dict::S('JSOFT_SOLUTION:InstallationProgress:DatabaseBackup'),
			'extensions_installation' => Dict::S('JSOFT_SOLUTION:InstallationProgress:ExtensionsInstallation'),
			'installation_successful' => Dict::S('JSOFT_SOLUTION:InstallationProgress:InstallationSuccessful'),
			'rollback' => Dict::S('JSOFT_SOLUTION:ConfigurationSafelyReverted'),
		),
		'authent' => $sUID,
	);
